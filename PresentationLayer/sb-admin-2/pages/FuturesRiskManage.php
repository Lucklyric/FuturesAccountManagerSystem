    ...
    function refreshData(flag) {
        ...
        if (flag === undefined) {
            if (sessionStorage.getItem('riskManagerData')) {
                riskManagerData = JSON.parse(sessionStorage.getItem('futuresRiskManagerData'));
        ...
        $.getJSON('../../../../FuturesAccountManagerSystem/BusinessLogicLayer/RiskManage/Refresh.php', function (data) {
            ...
            for (var i = 0; i < riskManagerTableData.length; i++) {
                if (riskManagerTableData[i][1].match(/\(期货/g)) {
                    riskManagerData.push(riskManagerTableData[i]);
                }
            }
            sessionStorage.setItem('futuresRiskManagerData', JSON.stringify(riskManagerData));
            console.log("取到数据");
            refreshMainTable();
        });
    }
    ...
    function sendRequest(event) {
        ...
        if (event === undefined){
            ...
            if ($('.nav-tabs .active').index() == 0) {
                ...
                data["风险度禁开线详细"] = ulToString($("#inDayPosUL"));
                data["隔夜减仓线"] = $("#overPos-totalLimit").val();
                data["隔夜减仓线详细"] = ulToString($("#overPosUL"));
...
// "净值日内限制","净值隔夜限制"
    ...
    function fillRiskModal(){
        ...
        if (riskGroupData[1].match(/普通\)/g, "")){//普通
            ...
            stringToUL(riskGroupData[9],$("#inDayPosUL"));

            $("#overPos-totalLimit").val(riskGroupData[12]);
            stringToUL(riskGroupData[13],$("#overPosUL"));
    ...
    //添加日内仓位设置
    $(document).on("click", "#inDayPos-add", function () {
        var inDaySetting = $("#inDayPos-instrument").val() + "-" + $("#inDayPos-limit").val();
        $("#inDayPosUL").append($("<li class='list-group-item'>").text(inDaySetting));
        updateCheckBox();
    });

    //删除日内仓位设置
    $(document).on("click", "#inDayPos-delete", function () {
        $("#inDayPosUL li.active").remove();
    });

    //添加隔夜仓位设置
    $(document).on("click", "#overPos-add", function () {
        var overSetting = $("#overPos-instrument").val() + "-" + $("#overPos-limit").val();
        $("#overPosUL").append($("<li class='list-group-item'>").text(overSetting));
        updateCheckBox();
    });

    //删除隔夜仓位设置
    $(document).on("click", "#overPos-delete", function () {
        $("#overPosUL li.active").remove();
    });
    ...
    //添加具体品种仓位设置
    $(document).on("click", "#abs-instrument-add", function () {
        var instrumentSetting = $("#abs-instrument").val() + "*" + $("#abs-limit").val();
        $("#abs-instrumentUL").append($("<li class='list-group-item'>").text(instrumentSetting));
        updateCheckBox();
    });

    //删除具体品种仓位设置
    $(document).on("click", "#abs-instrument-delete", function () {
        $("#abs-instrumentUL li.active").remove();
    });
    ...
    //添加具体品种仓位设置
    $(document).on("click", "#loss-instrument-add", function () {
        var instrumentSetting = $("#loss-instrument").val() + "*" + $("#loss-limit").val();
        $("#loss-instrumentUL").append($("<li class='list-group-item'>").text(instrumentSetting));
        updateCheckBox();
    });

    //删除具体品种仓位设置
    $(document).on("click", "#loss-instrument-delete", function () {
        $("#loss-instrumentUL li.active").remove();
    });
